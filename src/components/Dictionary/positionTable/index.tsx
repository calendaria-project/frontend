...
const PositionList: FC = () => {
  ...
  const save = async (record: IPositionViewModel) => {
    try {
      ...
      if (index > -1) {
        ...
        actionMethodResultSync('position', "put", getRequestHeader(authContext.token), newData[index])
          .then(() => {
            setData(newData);
            setEditingKey(-1);
          })
      } else {
        actionMethodResultSync('position', "post", getRequestHeader(authContext.token), record)
          .then(() => {
    ...
    },
    {
      ...
      render: (_: any, record: IPositionViewModel) => {
        ...
        return editable ? (
          ...
          <Button style={{ background: '#e6d87e' }} disabled={editingKey !== -1} onClick={() => edit(record)}>
            Изменить
          </Button>
        );
      },
  ...
  useEffect(() => {
    actionMethodResultSync('position', "get", getRequestHeader(authContext.token))
      .then(res => {
        setData(res.content);
      })
  }, []);
