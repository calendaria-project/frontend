...
const List: React.FC = () => {
    ...
    const searchAndUpdateChilds = (objOrArray: IItem[], id: number, children: IItem[]) => {
        ...
        for (let item in arrayOrObj) {
            if (arrayOrObj[item].id === id) {
                ...
                arrayOrObj[item].children = searchAndUpdateChilds(
                    arrayOrObj[item].children,
                    id,
                    children
                );
            }
    ...
    const GetAccordionItem = (item: IItem) => {
        ...
        return (
            <Panel header={item.id} key={item.id}>
                {isExistChildren ? (
                    <Collapse onChange={onChange}>{children.map(GetAccordionItem)}</Collapse>
                ) : (
                    <div></div>
                )}
            </Panel>
    ...
    return (
        <Content>
            <Row>
                <Col span={24}>
                    <Collapse onChange={onChange}>{data.map(GetAccordionItem)}</Collapse>
                </Col>
            </Row>
        </Content>
    );
